...
$Email_query="select * from user where User_Email='$email'";
$res=mysqli_query($conn,$Email_query);
    if (mysqli_num_rows($res)>0) {
        $user_error="<p>email already registered</p>";    
}
    else{
        $sql="insert into user(User_name,User_Address,User_Email,User_Mobile_No,User_Password) values('$name','$address','$email','$mob','$pass')";
...
//require ('register.php'); ?>
  <div class="wrapper">
        ...
        <form name="myform" onsubmit="return myscript()" action=""   method="post">
            ...
            <div><input type="submit" class="btn btn-dark"  name="submit">
            </div>
        </form>        
    </div>
