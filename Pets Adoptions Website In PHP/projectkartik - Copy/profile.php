...
$sid2=$_SESSION['ID'];
  $conn5=new mysqli('localhost','root','','pets_adoption');
          $sql5 = " SELECT *FROM shelter WHERE Shelter_ID='$sid2'";
                                                    ...
                                                    <div class="row m-l-0 m-r-0">
                                                        <div class="col-sm-4 bg-c-lite-green user-profile">
                                                            <div class="card-block text-center text-white">
                                                                ...
                                                                <h6 class="f-w-600"><?php echo $rec2['Shelter_Name'];?></h6>
                                                                <p><?php echo $rec2['Shelter_Adress'];?></p>
                                                            ...
                                                            <div class="card-block">
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-sm-6">
                                                                        ...
                                                                        <h6 class="text-muted f-w-400"><?php echo $rec2['Shelter_Email'];?></h6>
                                                                    </div>
                                                                    <div class="col-sm-6">
                                                                        <p class="m-b-10 f-w-600">Phone</p>
                                                                        <h6 class="text-muted f-w-400"><?php echo $rec2['Shelter_Contact'];?></h6>
                                                                    </div>
                                                                </div>
                                                                
                                                                <div class="row">
                                                                    <div class="col-sm-6">
                                                                        <p class="m-b-10 f-w-600">Requests</p>
                                                                        <h6 class="text-muted f-w-400"> <?php
                
                $sql01="select * from requests inner join shelter  where requests.Shelter_ID=$sid2 and requests.Shelter_ID=shelter.Shelter_ID";
                $result01=mysqli_query($conn5,$sql01);
                    ...
                    echo $count_active; ?>
                                                                    ...
                                                                    <div class="col-sm-6">
                                                                        ...
                                                                        $sql01="select * from accepted inner join shelter on accepted.Shelter_ID=shelter.Shelter_ID where accepted.Shelter_ID=$sid2 and accepted.Shelter_ID=shelter.Shelter_ID";
                                                                        $result01=mysqli_query($conn5,$sql01);
...
$sql10="UPDATE `shelter` SET `Shelter_Name` = '$username',`Shelter_Adress` = '$useradress', `Shelter_Email` = '$useremail', `Shelter_Contact` = '$userphone' WHERE `shelter`.`Shelter_ID` = $id2";
...
$conpass=$_POST['conpass'];
    $sql001="select * from shelter where Shelter_ID= $sid2";
    $res=mysqli_query($conn,$sql001);
    if($res->num_rows>0){
        $data=$res->fetch_assoc();
        if($data['Shelter_Password']===$currpass){
          if($newpass===$conpass){
          $sql002="UPDATE `shelter` SET Shelter_Password='$newpass' WHERE `shelter`.`Shelter_ID` = $sid2";
      ...
      ?>
      <div class="alert alert-warning alert-dismissible mt-5 fade fixed-top show" role="alert">
        <Span> current invalid password </Span> 
     <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> 
      ...
      <div class="modal-body">
        <form name="editform"  method="post" action="" enctype="multipart/form-data">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="name" name="name" value="<?php echo $rec2['Shelter_Name'];?> " pattern="[A-Za-z\s]{3,}"  required=""required="">
          </div>
          <div class="mb-3">
            <label for="recipient-name" class="col-form-label">Email:</label>
            <input type="email" class="form-control" id="email" name="email" value="<?php echo $rec2['Shelter_Email'];?>" required="">
          </div>
          <div class="mb-3">
            <label for="recipient-name" class="col-form-label">phone no:</label>
            <input type="number" class="form-control" id="phone" name="phone" value="<?php echo $rec2['Shelter_Contact'];?>" pattern="[0-9]{10}" required="">
          </div>
          <div class="mb-3">
            <label for="recipient-name" class="col-form-label">Address:</label>
            <input type="text" class="form-control" id="adress" name="adress" value="<?php echo $rec2['Shelter_Adress'];?> "required="">
          </div>
